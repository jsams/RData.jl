...
# if force_nullable is true, the result is always Vector{Union{T,Null}},
# otherwise it's Vector{T} if `rv` doesn't contain NAs
function jlvec(::Type{T}, rv::RVEC, force_nullable::Bool=true) where T
...
function jlvec(::Type{T}, rv::RNullableVector{R}, force_nullable::Bool=true) where {T, R}
    anyna = any(rv.na)
    if force_nullable || anyna
        res = convert(Vector{Union{T,Null}}, rv.data)
        anyna && @inbounds res[rv.na] = null
        return res
    else
        return convert(Vector{T}, rv.data)
    end
...
jlvec(rv::RVEC, force_nullable::Bool=true) = jlvec(eltype(rv.data), rv, force_nullable)
...
function jlvec(rl::RLogicalVector, force_nullable::Bool=true)
    ...
    if force_nullable || anyna
        return Union{Bool,Null}[ifelse(isna(x), null, x != 0) for x in rl.data]
    else
...
function jlvec(ri::RIntegerVector, force_nullable::Bool=true)
    isfactor(ri) || return jlvec(eltype(ri.data), ri, force_nullable)
    ...
    if force_nullable || anyna
        return CategoricalVector{Union{String, Null}, REFTYPE}(refs, pool)
    else
        return CategoricalVector{String, REFTYPE}(refs, pool)
    end
end
...
function sexp2julia(rv::RVEC)
    # TODO dimnames?
    # FIXME add force_nullable option to control whether always convert to Union{T, Null}
    jv = jlvec(rv, false)
    if hasnames(rv)
