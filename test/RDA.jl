module TestRDA
    ...
    @testset "NAs conversion" begin
        df = DataFrame(num = Union{Float64, Null}[1.1, 2.2],
                       int = Union{Int32, Null}[1, 2],
                       logi = Union{Bool, Null}[true, false],
                       chr = Union{String, Null}["ab", "c"],
                       factor = categorical(Union{String, Null}["ab", "c"], true),
                       cplx = Union{Complex128, Null}[1.1+0.5im, 1.0im])

        df[2, :] = null
        append!(df, df[2, :])
        df[3, :num] = NaN
        df[:, :cplx] = [null, Complex128(1,NaN), NaN]
